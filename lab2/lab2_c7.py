...
Original file is located at
    https://colab.research.google.com/drive/1f-6WKp3PyHqbsvvjUMaRmcbv3BwJdf2r
...
from resnetNoBn import ResNet18
import numpy as np
import random
import os
import argparse
...
parse.add_argument('--cuda', default = 'gpu', type = str, help = 'cuda')
parse.add_argument('--num_workers', default = 12, type = int, help = 'num_workers')
...
if opt == 'sgd':
    ...
    optimizer = torch.optim.Adagrad(net.parameters(), lr=lr, weight_decay=5e-4)
elif opt ==  'adadelta':
...
def train(epoch,num_worker):
    ...
    print('[Train]: Epoch: %d | num_worker: %d | Loss: %.3f | Acc: %.3f%% | time_training: %.4f'% (epoch+1, num_worker,train_loss/(100), 100.*correct/total,time_training))
    ...
    return train_loss,time_training

run_time = []
train_loss_list = []
print('No Batch Norm layer')
for epoch in range(0, 5):
    t_loss,r_time = train(epoch,num_workers)
